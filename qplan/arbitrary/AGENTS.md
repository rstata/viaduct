...
Run `./gradlew :arbitrary:test` for generator validity and configuration tests. The resolver01 and resolver02 property tests in `semantics` are the end-to-end judgment: they parse and flatten each generated query through the test fixture, apply the semantic resolver constructor, and require the result to satisfy `correctResolution`.
