...
Field collection is deliberately downstream of this equivalence. Duplicate obligations remain distinct, so any future collector that depends only on these resolution observations may group equal `Value.Key` values, combine child demand, or retain duplicates without invalidating the argument. A collector that depends on erased information such as aliases or response order is outside the claim because it performs field completion rather than field resolution keyed by `Value.Key`.
