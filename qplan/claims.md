...
**[field-only-node-lowering]** Within the fixture-supported node domain, a retained source GraphQL-Java schema plus a separate model-only schema with `foo_V_A_node` producers, hierarchy-preserving `T_V_A_Bridge` records, and concrete-object `node` loaders preserves the field-resolution obligations of external node-valued field and node-resolver inputs.
