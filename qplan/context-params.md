...
The current world surface includes `world.schema`, `world.executorRegistry`, `world.selectiveResolvers`, `world.behavioral(...)`, and `world.selectionsFrom(...)`. Value factories belong to their precise `Value` variants rather than to `Schema` or `Assumptions`.
