...
This inventory records the current synchronization boundary between qplan and the 32 `core/engine/runtime` feature-test files selected for this porting surface. Fifteen qplan port files exist and 17 source files remain whole-file exclusions or migration candidates.
...
Fourteen ports currently match the source test names and counts. The synchronization gap below keeps the remaining port from being a complete whole-file migration.
...
- **Root-field references:** All 23 source tests are copied. Ten remain disabled under explicit reason labels: four `AccessChk`, three `CallerAttr`, one `NodeLower`, one `ALT: RefDedup`, and one `ALT: RootObjRSS`. Reference resolution otherwise runs through Resolver26. The object-RSS case remains source-faithful and disabled; its enabled `ALTERNATIVE` prefixes the namespace path and reads the same data through Query RSS. Production's equivalent-reference deduplication test likewise remains disabled; its enabled `ALTERNATIVE` expects one fresh target execution for each of the three reference occurrences.
- **Execution:** `TestWorld` fills missing nullable Query fields with null producers and missing non-null Query fields with error producers. Node references normalize to `Query.node`; its concrete-type dispatcher treats the originating ID as authoritative when composing raw lookup data, matching production's `NodeEngineObjectDataImpl`, so the lookup payload need not repeat it. Seven node tests currently pass through qplan; six remain disabled.
...
- **Variables:** `VariablesResolverTest.kt`'s disabled `variables are coerced` preserves production's independent object- and Query-RSS bindings for the same variable name. Qplan gives each resolver variable name one occurrence-local binding; its passing `ALTERNATIVE` uses distinct `varx` and `vary` names while preserving the coercion behavior under test. The adapter also preserves production bootstrap validation for lossy abstract-type traversal by translating canonical provider-path rejection into `InvalidVariableException`; the union traversal validation test now runs unchanged.
- **Selective fields:** The executor adapter now passes converted Resolver26 demand to selective field executors and honors configured field-selectivity providers. Eighteen of the 71 formerly `Selective`-only cases in `SelectiveFieldResolversExecutionTest.kt` and three of the seven such cases in `RequiredSelectionsTest.kt` run unchanged. All eighteen production tests formerly labeled `SelSem` retain their source form as disabled `ALT` tests and have passing one-shot `ALTERNATIVE` counterparts. The alternatives preserve final GraphQL data and meaningful error paths while replacing repeated materialization, snapshot reconciliation, passive argument-bearing output, runtime-type-dependent demand narrowing, and surplus-output policy with Resolver26's closed-demand semantics. The six inherited arbitrary suites instantiate a generated production `Viaduct` directly and expose no `EngineTestModule`, matching the selective-node arbitrary-suite N/A classification rather than representing adapter blockers. Other disabled cases are classified by their actual non-`SelSem` blocker.
- **Selective-field batching:** The two production tests intended to verify batching remain disabled unchanged with `TODO: Batching`. They should be enabled unchanged only after qplan implements batching.
- **Selective-field instrumentation:** The two remaining instrumentation-only cases assert production materialization source identity or two staged field-fetch callbacks. They carry no additional resolver-result assertion and are N/A for Resolver26's one-shot correctness boundary, consistent with the omitted instrumentation feature-test files.
- **Selective-node batching:** The four production tests intended to verify batching remain disabled unchanged with `TODO: Batching`. They should be enabled unchanged only after qplan implements batching.
- **Selective-node consistency alternatives:** The embedded-list, top-level, and nested runtime-type drift tests, inconsistent-exception test, and malformed nested-replacement test retain their production refetch-comparison forms as disabled ALTs. Their alternatives preserve the same GraphQL nulls, diagnostic message class, and consumer paths from one stable node result plus a consumer-local failure, and each verifies one node call rather than fabricating contradictory producer results.
- **Selective nodes:** Selective node executors are lowered to selection-sensitive synthetic payload resolvers and receive Resolver26's closed one-shot demand. Twenty-seven production tests in `SelectiveNodeResolversExecutionTest.kt` run unchanged, covering concrete and abstract lookup, lists, nested node-field demand, merged demand, static directive behavior, RSS coverage, recursive demand, embedded lists, alias recovery, engine-managed id-only demand, recursive id-only self-reference, nullable selected-field omission, and field-resolver ownership of selective-node surplus. The adapter projects selective node output before it enters qplan, dropping undemanded top-level fields and fields owned by registered field resolvers and filling omitted demanded nullable fields with null. The four production batching tests remain disabled unchanged until qplan implements batching. The remaining production cache/refetch assertions describe repeated materialization rather than missing selective-node invocation; checker, instrumentation, subquery, custom-directive, and argument-bearing node-output shapes retain specific exclusions. The inherited arbitrary suites execute a generated production `Viaduct` directly and do not expose an `EngineTestModule` adapter surface, so they are N/A for this harness.
- **Selective-node one-shot alternatives:** `materialization output selections preserve path-specific concrete ownership` preserves the response and one producer call while dropping the production second-materialization snapshot, whose demand is runtime-narrowed after the first fetch. `initial materialization resolves node reference` retains production's post-query mutation assertion on its original `NodeEngineObjectData`; its alternative verifies the same GraphQL data without inspecting that production cache object. `reused node reference resolves newly selected fields` replaces production's field accumulation on one mutable reference with fresh closed reference occurrences. `missing nested rss rematerializes node source` retains production's two-fetch assertion; its alternative verifies the same nested RSS result from Resolver26's one closed-demand node call. `surplus coverage uses values from the first covering result` replaces successive cache candidates with one stable result carrying the same consumed values. `changed embedded list size leaves unmatched items unresolved` publishes one stable two-item list with the same successful first value and explicitly errored second dependency instead of reconciling differently sized fetches. `client and rss arguments remain isolated` represents the two argument tuples as active field keys instead of separate node materializations, and `rss variable failure reports a field error` preserves the provider failure and consumer path with the argument-bearing RSS field actively owned. The two embedded argument-variable alternatives likewise move `Foo.bar(y:)` and nested `Bar.y(x:)` into active field resolvers, preserving the ancestor and fragment argument results without admitting argument-bearing node output.
...
- **Scoped schemas:** The adapter builds Resolver26's world from the full schema while validating and completing the public operation against an optionally distinct scoped executable schema. `RequiredSelectionsTest.kt`'s private-field RSS case now runs unchanged.
- **Subquery schemas:** Both Query `ctx.query()` schema-selection tests run unchanged, including access to a full-schema-only field from an operation completed against a scoped schema. The `ctx.mutation()` case remains disabled as `Mutation`.
- **Subquery execution:** Eighteen of the 27 source tests run unchanged, covering nested calls, resolver selection sets, aliases, arguments, variables, fragments, variable isolation, object-RSS reads, selective output, and root-type rejection. Seven mutation tests remain disabled as `Mutation`; the production metric assertion is N/A; and production's QueryPlan exception-wrapper assertion has an enabled `ALTERNATIVE` that verifies qplan's native schema failure.
...
The current enablement pass began with 43 `TODO` annotations after excluding `AccessChk`, `ParentFld`, and `ErrorData`. Nine copied tests now run unchanged, fifteen production forms now have passing one-shot `ALTERNATIVE` tests, and eight stale TODOs were reclassified N/A because they are production arbitrary/instrumentation harnesses with no qplan adapter surface. Eleven remain: six batching tests, three mutation-root tests, and two custom resolver-directive spelling tests. Batching-focused tests remain disabled unchanged until qplan implements batching; the other two categories require model/semantic support rather than execution-adapter wiring and were not changed in this pass.

Counts overlap because one test may be blocked by more than one requirement. `Batching` and `Directive` record the eleven retained TODOs from this enablement pass; the other rows summarize actionable mixed-feature cases in `EngineFeatureTestExample.kt`, `FromFieldVariablesFeatureTest.kt`, `RequiredSelectionsTest.kt`, `SubqueryExecutionTest.kt`, `SubquerySchemaTest.kt`, and `VariablesResolverTest.kt`. Dedicated ports whose primary unsupported surface is already evident from the file are otherwise excluded. Labels appear space-separated in actionable `@Disabled("TODO: ...")` reasons; `IntentDiff` identifies the fifteen intentional incompatibilities whose specific `ALT` prose reasons are retained. `AccessChk` includes checker and type-checker executors together with their object- and Query-rooted required selections; checker Query fragments carry no additional blocker. Resolved categories and N/A behavior are omitted rather than retained as zero-count blocker rows.
...
| Batching | 6 | `Batching` |
| Likely mechanical adapter enablement | 2 | `MechAdapt` |
| Abstract-type/runtime applicability | 1 | `Abstract` |
| Directives | 2 | `Directive` |
| Mutations | 11 | `Mutation` |
| Rich executor error preservation | 1 | `ErrorData` |
| Intentional semantic incompatibilities | 15 | `IntentDiff` |
