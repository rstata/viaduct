...
The `__typename` lowering checkpoint is complete. The unchanged source GraphQL schema retains GraphQL's system `__typename`, while the lowered qplan schema contains the synthetic interface `V_I_Top { V_I_typename: String! }`, an owner-specific ordinary `V_I_typename` field on every lowered object and source interface, no field on unions, and every lowered object in `V_I_Top.possibleTypes`. Source names containing `V_I` are reserved alongside `V_A`. External operation translation recursively erases `__typename` selections but preserves selected composite parents with empty lowered subselections; GraphQL Java's system data fetcher completes client typename fields from the concrete `ObjectEngineResult.type`. Internal resolver object fragments lower object- and interface-scoped typename to the owner field, union-scoped typename to `V_I_Top.V_I_typename`, and Node-valued paths through the existing bridge payload. Each concrete object field has an ordinary generated constant resolver with no arguments, object-fragment demand, variables, or dependency edges. Resolver invocation no longer synthesizes typename recursively, the root EOD is empty, resolver outputs retain no discriminator, and correct-resolution reasoning relies on ordinary resolver conformance rather than a distinguished typename predicate.
