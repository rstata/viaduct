...
GraphQL-Java remains the source-facing schema representation, while qplan's lowered `Schema` is used exclusively for field-resolution reasoning. The retained source `GraphQLSchema` is unchanged and never contains synthetic bridge or typename-lowering definitions. Fixture decoding and lowering share one `GraphQLTypeRelations` instance built from that source schema; qplan does not define a parallel type-relation API. Output fields directly own their `args` collections, and each canonical `Schema.FieldArg` points back to its owning field; qplan has no synthetic field-argument definition object. `Schema.CompositeTypeDef.possibleObjectTypes` remains transitional model data: source composite types are populated from the shared relation helper, every source Node object or interface has a same-kind bridge whose possible object types mirror the source implementation hierarchy, and the synthetic `V_A_AllSourceObjects` contains every source-derived object type while excluding node-bridge objects. Runtime node references materialize concrete object bridges even when returned through abstract source fields. Each source-backed lowered `Schema.Object` retains the exact `GraphQLObjectType` from the source schema as its Engine API witness. Synthetic node-bridge objects retain generated witnesses only for internal EODs. Access those attachments only through `gjDef`; do not inspect them for model reasoning or expose synthetic coordinates or values through tenant-visible APIs.

`EngineObjectData.Sync` is name-keyed, untyped at the value boundary, and partial. `get` distinguishes an unset selection by throwing, `getOrNull` tolerates it, and `isPresent` distinguishes absent from present-null without reading the value. Qplan's implementation validates values against qplan's canonical lowered schema during construction and retains that canonical `Schema.Object` for context-free semantic access. Source-backed EODs expose the exact source `GraphQLObjectType`; tenant-visible materialization recursively unwraps node bridge values, including abstract and list-valued fields. Fixture composition recursively adapts tenant-produced source EODs into qplan-owned lowered EODs before semantic reasoning.
...
6. Complete, EOD carrier migration: qplan owns a validating `EngineObjectData.Sync` implementation with canonical passive-field and alias-preserving construction paths. Source-backed objects expose exact retained source GraphQL-Java witnesses, generated synthetic witnesses remain internal, tenant-visible materialization removes node bridges, source resolver outputs are adapted into canonical lowered values, and all former `Value.Object` call sites use EOD without changing OER keys, cells, occurrence identity, response-key materialization, or access-result semantics.
