...
`FromArgument` variables are stamped at the defining resolver occurrence, bound from its exact arguments in request-local monotonic state, and grounded during layer-by-layer local demand closure. Resolver10 also evaluates compiled `FromObjectField` providers against published OER values and grounds the resulting pending selections.
...
The next executor milestone is coroutine-driven resolution over deferred value promises. `RuntimeSupport` now provides thread-safe writer registration and monotonic resolver-read cycle detection, and contextual materialization checks each exact OER-and-ground-key read before awaiting its promise; promises themselves carry no writer metadata. Resolver21 orchestration should preinstall and register every demanded promise, launch resolver coroutines beneath one root scope, and let suspending materialization retain traversal continuations instead of repeatedly rescanning dependency trees. Cancellation originates at the root, while an uncaught child failure fails the root scope. Cycle detection diagnoses violated dependency assumptions rather than general coroutine deadlock.
...
Run `./gradlew check` for the Kotlin model and documentation labels. Resolver09 and Resolver10 have opt-in fixed-seed stress tasks named `resolver09Stress` and `resolver10Stress`. Generated tests choose and report explicit seeds; a green run is finite evidence, not a timeless repository fact. The TLA+ toolchain and complete validation matrix are documented in [`tla/README.md`](./tla/README.md).
