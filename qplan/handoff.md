...
The intended execution state is monotonic. An OER cell or variable binding moves only from absent to one validated value and is never replaced. Parent cells may publish mutable child OERs before those children are complete, so descendants can gain cells without rebuilding ancestors. A future executor should preserve this `unset -> set` model while replacing the current fixed depth-first continuation with readiness-driven scheduling.
...
Run `./gradlew check` for the Kotlin model and documentation labels. Generated tests choose and report explicit seeds; a green run is finite evidence, not a timeless repository fact. The TLA+ toolchain and complete validation matrix are documented in [`tla/README.md`](./tla/README.md).
