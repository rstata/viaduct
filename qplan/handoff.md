...
GraphQL-Java remains the source-facing schema representation, while qplan's lowered `Schema` is used exclusively for field-resolution reasoning. The retained source `GraphQLSchema` is unchanged and never contains synthetic bridge definitions. Each lowered `Schema.ObjectType` separately owns one canonical opaque `GraphQLObjectType` integration witness: an unchanged source definition when its field surface matches, or a generated definition for transformed owners and synthetic bridges. Access that witness only through `gjDef`; do not inspect it for model reasoning or expose bridge coordinates through tenant-visible APIs.
