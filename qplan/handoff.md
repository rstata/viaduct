...
The next executor milestone is coroutine-driven resolution over deferred value promises. Orchestration should preinstall every demanded promise, register its exact OER-and-ground-key slot with the writer's root-relative path in `RuntimeSupport`, launch resolver coroutines beneath one root scope, and let suspending materialization retain traversal continuations instead of repeatedly rescanning dependency trees. Cancellation originates at the root, while an uncaught child failure fails the root scope. `RuntimeSupport` still needs monotonic resolver-read cycle detection over those registrations; promises themselves carry no writer metadata. This detects violated dependency assumptions rather than general coroutine deadlock.
