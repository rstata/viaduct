...
Resolver21-23 now establish coroutine-driven resolution over deferred value promises for the full Resolver01-03 value-resolution domain and output policies. The shared constructor preserves install-before-launch, child-install-before-publication, exact reader-coordinate, root cancellation, and structured quiescence properties across all three wrappers. Promise-backed `FromArgument` bindings establish the variable lifecycle for Resolver24; coroutine suspension on unresolved `FromObjectField` bindings, pending symbolic selections, late grounding, and demand sealing remain its next work. Arbitrary-dispatcher behavior remains outside this coroutine family. Cycle detection diagnoses violated dependency assumptions rather than general coroutine deadlock.
