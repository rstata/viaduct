...
Resolver03 and Resolver04 no longer compute transitive input demand with runtime `closeResolverDemand` functions. Each activated resolver contributes its exact registry-computed `extendedFragment` in one pass; Resolver04 then resolves variables whose registered defining fields belong to the current concrete object type, instantiates those variables across the applicable selections and input closure, and preserves the remaining variables symbolically. The separate `subselections.withExtendedResolverDemand()` call remains the output-side operation that retains client demand and the prerequisites of successor resolver occurrences.

For an exact resolver occurrence `R(arguments)`, its `extendedFragment(arguments)` is defined role-neutrally as the guarded, path-rooted transitive closure of `R`'s `objectFragment(arguments)` under resolver-dependency expansion. On the input side, that closure supplies the prerequisites needed to construct `R`'s input. On the output side, it extends producer demand with the data required by successor resolver occurrences.

Implementation logic changed only in [`semantics/resolver03/Resolver.kt`](./semantics/src/main/kotlin/semantics/resolver03/Resolver.kt) and [`semantics/resolver04/Resolver.kt`](./semantics/src/main/kotlin/semantics/resolver04/Resolver.kt). Documentation and KDoc were updated for the role-neutral closure model. No TLA+ artifact or proof changed, so the TLA+ construction remains intentionally stale with respect to this Kotlin simplification.
...
[`resolver03`](./semantics/src/main/kotlin/semantics/resolver03/Resolver.kt) filters applicable selections and adds each activated resolver field's exact `extendedFragment(arguments)` as its input closure before dependency ordering, materialization, and recursive resolution. Separately, before applying a field resolver, it walks the output demand and roots each encountered successor resolver's exact extended fragment at that occurrence. This guarded output expansion makes passive transitive requirements visible to the producing resolver before `snipToDemand` projects its output. Generated node loaders use the same operations, including exact synthetic bridge argument tuples, and do not introduce a distinct semantic rule.
