...
Resolver26 closes each OER once. Successor demand statically transposes descendant parent demand into selective producer output, and each returned OER adds only that lifted portion to its initial input closure. This makes all ancestor work discoverable before child construction while retaining source-sensitive suppression of standard resolver demand. Every OER freezes after its one local launch cycle.
