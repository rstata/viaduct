...
The next executor milestone is coroutine-driven resolution over deferred value promises. Orchestration should preinstall every demanded promise, launch resolver coroutines beneath one root scope, and let suspending materialization retain traversal continuations instead of repeatedly rescanning dependency trees. Cancellation originates at the root, while an uncaught child failure fails the root scope. `RuntimeSupport` still needs monotonic resolver-read cycle detection using each deferred promise's writer-coordinate stamp; this detects violated dependency assumptions rather than general coroutine deadlock.
