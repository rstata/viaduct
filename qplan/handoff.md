...
Resolver21-23 now establish coroutine-driven resolution over deferred value promises for the full Resolver01-03 value-resolution domain and output policies. The shared constructor preserves install-before-launch, child-install-before-publication, exact reader-coordinate, root cancellation, and structured quiescence properties across all three wrappers. The next concurrent-executor scope is not yet fixed; runtime `FromObjectField`, pending symbolic selections, late grounding, demand sealing, and arbitrary-dispatcher behavior remain outside this coroutine family. Cycle detection diagnoses violated dependency assumptions rather than general coroutine deadlock.
...
Run `./gradlew check` for the Kotlin model and documentation labels. Resolver03, Resolver08, Resolver09, Resolver10, and Resolver23 have opt-in fixed-seed stress tasks named `<resolver>Stress`; `run-resolver-stress.sh` launches all five. Generated tests choose and report explicit seeds; a green run is finite evidence, not a timeless repository fact. The TLA+ toolchain and complete validation matrix are documented in [`tla/README.md`](./tla/README.md).
