...
- `establishAssumptions()` initializers for globals such as `variableValues` and `schema` denote stipulated inputs. They are not unfinished startup code. Every reasoning exercise must state the relevant contents of those globals, and every occurrence of a global denotes that same fixed value throughout the exercise.
...
- Schema definitions intentionally form reciprocal graphs: type definitions contain their fields, while fields and arguments navigate back to their containing definitions. Ordinary immutable Kotlin construction cannot instantiate these classes while simultaneously satisfying those invariants. Reasoning assumptions stipulate the complete canonical definition graph represented by the global `schema`; the classes in `Schema.kt` are not an executable construction API.
- Nesting definition declarations under `Schema` provides namespacing only; a nested definition object does not retain an enclosing `Schema` instance. Same-schema ownership and canonicality are stipulated invariants that Kotlin does not enforce.
