...
- Each reasoning exercise has exactly one `Assumptions` and one `Schema`. Construct every `Schema.Value` and `Schema.ArgumentsValue` through that schema's instance factories; those factories enforce canonical schema references at the construction boundary, so reasoning code may assume that definitions carried by values belong to the one schema without repeating ownership checks.
