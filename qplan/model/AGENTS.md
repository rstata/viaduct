...
- Each reasoning exercise has exactly one `Assumptions` and one `Schema`. Construct every `Schema.Value` other than the schema-independent `Schema.ErrorValue`, every `Schema.ArgumentsValue`, and every `Schema.ObjectKey` through that schema's instance factories; those factories convert host values according to their schema input types. The one-schema discipline stipulates that every supplied definition and non-error value belongs to that schema, so factories do not repeat ownership checks.
...
- `FieldValues`, `ObjectFieldValues`, and `VariableBindings` deliberately throw from `get` when a field, object key, or variable is missing. Map extension helpers such as `getOrElse` may call that throwing operation rather than applying their fallback. Check `containsKey` explicitly when absence is possible.
...
- `Schema.ObjectValue` fields are keyed by exact `Schema.ObjectKey` coordinates rather than field names. This preserves distinct coerced argument tuples for one field in an alias-free resolved object.
...
Unlike the existing Viaduct implementation, the model's shared `Schema.ObjectKey` does **not** include a response alias. OER and resolved-object field identity is determined by the canonical `Schema.OutputField` and its fully coerced arguments. Selections of that canonical field with different aliases therefore address the same OER cell or `Schema.ObjectValue.fieldValues` entry; fields with the same name at distinct schema coordinates remain distinct. Aliases remain relevant when constructing the external GraphQL response, not when identifying field-resolution data.

Every key present in an `ObjectEngineResult` or `Schema.ObjectValue` carries a field whose `containingType` is that value's concrete `Schema.ObjectType`; neither value contains a key for an interface or union field coordinate. Keys used outside those values, including `Selection.key`, may carry abstract-type fields or unresolved variables. Such a key must be specialized to the applicable concrete object field and have its variables instantiated before it is present in an OER or object value.
