...
The executor registry exposes demand between concrete `Schema.ObjectField` coordinates. During registry assembly, a private dependency graph also contains `Value.Variable` vertices so providers can be ordered before variable uses and cycles through variables can be rejected. A variable is structurally identified by its name, the canonical concrete object field whose resolver defines it, and a nullable exact OER path reserved for occurrence-specific alpha-renaming; registry assembly currently constructs resolver-level variables with null paths and establishes canonical ownership and global name uniqueness.
