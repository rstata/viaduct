...
A variable template is identified by its local name and defining concrete resolver field. Its `instanceId` is null. Instantiating it for an exact `ResolverOccurrenceId` creates an occurrence-specific variable carrying a `VariableInstanceId`. Every use of one variable within that resolver occurrence has the same identity, including uses in different selections or descendant OERs. Request-local `Assumptions` stores one declared promise per variable-instance ID: synchronous semantic operations read completed bindings, while coroutine operations may suspend for them.
