...
Resolver24 extends the coroutine family through `CoroutineOpenResolve.kt`. One persistent orchestrator per OER exclusively grounds variable-bearing key arguments, interns each resulting `GroundKey`, and creates at most one slot producer for it. Expanding an exact resolver occurrence declares all of its `FromObjectField` promises and launches one definition-owned provider reader; provider slots do not know which variables they supply. Materialization and provider traversal submit open keys through their OER orchestrators, while conservative projection demand and complete-output retention preserve late-key convergence and demand entering published child OERs.
...
`resolver24i/Resolve.kt` is a presentation-oriented implementation for one narrow case: selective object-fragment resolvers with both `FromArgument` and `FromObjectField` variables and selective output. It specializes the public wrapper, open coroutine orchestration, variable expansion, demand sealing, provider conversion, and passive result construction into one source file while leaving model carriers, promises, runtime cycle checking, and registry ground-boundary demand derivation shared. Complete resolver values stay internal for late path reads; after quiescence, every OER is projected to the exact demand accepted by its orchestrator, preserving the transitive resolver-input closure without exposing unrelated passive output.
...
Resolver01, its queue-backed counterpart Resolver06, and coroutine-based Resolver21 support empty user object fragments plus generated bridge loaders. Resolver02, its queue-backed counterpart Resolver07, and coroutine-based Resolver22 support nonempty fragments and `FromArgument` with complete output. Resolver03, its depth-first queue counterpart Resolver08, readiness-driven Resolver09, and coroutine-based Resolver23 support that domain with selective projection. Resolver10 and coroutine-based Resolver24 additionally support runtime `FromObjectField`; Resolver25 supports a strict direct-scalar-sibling subset through one-shot field preparation. Resolver01/02/06/07/21/22 use complete resolver outputs, while Resolver03/08/09/23/25 use full `successorDemand()` and selective projection; Resolver10/24 retain complete output around open demand so late exact keys can be serviced without repeating resolver applications.

Resolver01/02/06/07/21/22 require `Assumptions.selectiveResolvers == false`; Resolver03/08/09/10/23/24 require it to be true. Use that world flag for resolver invocation and passive output traversal rather than carrying another selectivity flag in completion state.
