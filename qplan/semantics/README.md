...
All maintained resolvers use `shared/SharedResolvePassiveValues` plus `SharedOrchestrationTask`, `SharedFieldResolverContext`, `SharedFieldResolverTask`, and `SharedTaskDispatcher`. An orchestration-domain factory closes demand and initializes task state; passive resolution fills source-owned fields before dispatching that prepared task. Resolver26 uses request-root coroutines and promises for readiness. The six depth-first resolvers use the same grounded task implementations under `resolvers/resolver01`: their orchestration factory closes grounded demand, orchestration dispatches fields in sibling dependency order, and each field task claims its publication before dispatch and completes it after passive resolution.
