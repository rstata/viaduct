...
`OperationContext` is the stable reference bundle for this scope. It extends `SharedOperationContext<TaskDispatcher>`, retains the task dispatcher and Resolver26 observer, and exposes two independent mutable protocols as properties: `cycleChecker: CycleCheckState` and `bindingDeclarationsState: BindingDeclarationsState`. The context neither implements those protocols nor owns their mutable storage.
