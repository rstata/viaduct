...
Every resolver key in closed demand is represented either by the expansion map or by an argumentless source-provided field. Successor-demand construction closes parent selections statically before returned objects exist. Each returned object's initial closure combines its construction demand with only the demand transposed into that object from descendant parent selections. Repeating this one-level static lift handles grandparents without recursively traversing parent backedges as structural children or reopening an ancestor task.
