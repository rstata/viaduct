...
Resolver observations record both the stable publication identity and every independently rooted invocation. Correctness replay re-evaluates the source-owning resolver, follows the recorded reference-hop chain, re-evaluates each target with its empty object input, arguments, demand, and Query witness, and finally checks the ordinary output at the consumer position. Application accounting records each target directly; a reference identity root contains no executable namespace occurrences or object-fragment dependencies to traverse.
