...
class ArgumentStampingTest {
    ...
    fun `equal pre-grounded selections merge in fragments and external queries`() {
        ...
        val resolvedResult =
            context(world) {
                world.objectOf("Query").resolve(resultQuery.subselections)
            }

        assertEquals(Value.Int.of(8), resolvedResult.getValue(resultKey).get())
        assertEquals(1, frankApplications)
        assertEquals(listOf(setOf("one", "two")), frankDemandFields)
        assertTrue(context(world) { resolvedResult.correctResolution(resultQuery) })
        ...
        val oneKey =
            Value.GroundKey.of(
                world.schema.objectField("Payload", "one"),
                emptyMap(),
            )
        val twoKey =
            Value.GroundKey.of(
                world.schema.objectField("Payload", "two"),
                emptyMap(),
            )
        val externalQuery =
            world.fragmentFrom(
        ...
        val resolvedExternal =
            context(world) {
                world.objectOf("Query").resolve(externalQuery.subselections)
            }
        val payload = assertIs<EngineResult.Object>(resolvedExternal.getValue(frankKey).get())

        assertEquals(Value.Int.of(3), payload.getValue(oneKey).get())
        assertEquals(Value.Int.of(5), payload.getValue(twoKey).get())
        assertEquals(2, frankApplications)
        assertEquals(
            listOf(setOf("one", "two"), setOf("one", "two")),
            frankDemandFields,
        )
        assertTrue(frankKey !is Value.GroundKey.Stamped)
        assertTrue(context(world) { resolvedExternal.correctResolution(externalQuery) })
    }
