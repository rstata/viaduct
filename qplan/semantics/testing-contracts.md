...
- `NodeResolverContract` covers source-level node resolution by retaining Node-valued field coordinates, normalizing their outputs to `Query.node` references, dispatching each reference by the concrete type encoded in its global ID, and resolving lists element by element. Resolver26 implements it.
...
| `node` | Node values normalized to `Query.node` references with concrete-type dispatch | Resolver26 | `10:3:5` |
