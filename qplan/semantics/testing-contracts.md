...
- Resolver26's `RootFieldReferenceResolutionTest` covers symbolic root-field references at object fields and list elements, registered-resolver override, grounded arguments, scalar and enum targets, concrete and abstract composite targets, namespace nesting, Query fragments and their variables, reference tails, and successor demand below referenced results.
...
| `selective-node` | Selection-aware fixture-lowered nodes with model-owned output projection and supplied-demand evidence | Resolver26 | `10:3:5` |
...
The Resolver26-only `selective-node` profile retains the complete deterministic generated node program as its value oracle. Its node callback receives payload demand through `ofSelectionAwareNonselective`, model-owned projection produces the demanded partial output, and an independent application witness requires that an activated lowered node loader received nonempty supplied demand. The deterministic model fixture test separately verifies that the node-lowering adapter passes the exact payload demand into the callback.
