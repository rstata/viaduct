...
The proof separates demand discovery, same-object dependency availability, and nested result construction. Lemma 1 establishes that the selection forest used to construct an object contains every transitively required resolver selection. Lemma 2 establishes that the dependency-ordered LIFO transitions can supply each field resolver with the object fragment selected by those requirements. Lemma 3 establishes that resolving each selected value satisfies its accumulated subselections and recursively closes every nested result.

For the final Query result, consider any present key. If its arguments contain an error or its field has no registered field resolver, `isClosedUnderResolverDemand` imposes no field-resolver-fragment obligation at that key. Otherwise, Lemma 1 places every applicable selection from the field resolver's stamped and grounded object fragment in the closed selection forest, and Lemma 3 shows that the completed OER satisfies those selections. Lemma 3 also shows that every value below the key is recursively closed. Synthetic `$id` and `$ids` bridge requirements introduced by fixture lowering are ordinary field-resolver fragment obligations using generated `FromArgument` variables, so the same argument covers generated node loaders without a separate correctness rule. Therefore every local resolver obligation observed by `isClosedUnderResolverDemand` holds throughout the result tree.
...
The argument is an induction over the prefix of the ordering returned by `dependencyOrder`. A key is ready only when `dependenciesOf` finds no required sibling among the unresolved keys. Lemma 1 guarantees that every applicable required sibling belongs to the complete key set, so a required sibling absent from the unresolved suffix must already be written in the partial OER. The prefix induction invariant, using Lemma 3 for previously resolved keys, says that each such sibling cell also satisfies its accumulated subselections. The partial OER therefore conforms to the current resolver's stamped and grounded object fragment, making materialization defined. For a fixture-generated loader at singular `foo(args)`, generated argument variables ground the fragment requirement to `foo$id(args)`; a list-shaped `foo(args)` similarly requires `foo$ids(args)`. Unequal argument tuples remain separate dependencies.
...
The argument is a mutual induction over passive `resolveValue` construction and queued object work. Null, error, and simple values satisfy the claim immediately because they create no descendant demand. A list applies the induction hypothesis independently to every element while treating list-transparent child work uniformly across list positions. For an object, non-selective construction copies every actually supplied passive field recursively and stops at registered resolver boundaries, while resolver demand creates the OER items that must resume active resolution. The finite returned value itself bounds nested construction. If newly closed incoming demand reaches beneath an existing passive key, Resolver02 creates work from the same source value. Resolver keys already written on the partial OER are excluded from its ordered work; rebuilding a separate passive subtree can resolve descendants again, which is permitted because this theorem makes no invocation-schedule claim.

For an object at the root of an object `resolve` application, Lemmas 1 and 2 establish every activated field-resolver obligation, while the induction hypotheses establish closure below every cell. There is no object-type resolver activation or implicit root bridge obligation in the canonical model. A fixture-generated node loader is simply a field resolver whose input fragment activates its matching synthetic bridge coordinate, and queued object resolution handles its loaded object by the same induction case as every other object value.
