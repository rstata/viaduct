...
The argument is a mutual induction over the finite derivation of `resolveValue` and object `resolve`. Null, error, and simple values satisfy the claim immediately because they create no descendant demand. A list satisfies it by applying the induction hypothesis independently to every element. An object groups applicable selections by concrete key, constructs each required cell, and applies the induction hypothesis to that cell's merged subselections. At a resolver application, the private uniform `outputSelectionForest` expansion supplies every non-behavioral field along acyclic paths and unfolds a recursive ownership path only while the corresponding locally closed demanded subselections remain nonempty. Consequently the public selective resolver result contains every passive value that the subsequent recursive closure can consume in this non-selective case.
