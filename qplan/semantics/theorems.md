...
This file records informal theorem statements and arguments about the semantic Kotlin model. The scoped TLA+ baseline in [`../tla`](../tla) machine-checks a finite atomic construction calculus under explicit extraction and alignment assumptions; it is not a proof of these Kotlin functions.
...
Filtering and concrete-key specialization through `SelectionForest.merge(type)` agree with the corresponding operations in `conformsToSelections`. Thus the closure contains the exact concrete keys whose cells are needed to satisfy each applicable resolver fragment, rather than merely abstract or inapplicable selections.
...
The argument is a mutual induction over passive `resolveValue` construction, retained object continuations, and object resolution. Null, error, and simple values create no descendant demand. A list applies the induction hypothesis independently at every exact list-index path. For an object, passive construction allocates one mutable target OER, copies supplied passive cells, stops at registered resolver boundaries, and records the source, target, path, and collapsed selections for each occurrence needing active work. `resolveObjects` later completes those retained targets deepest first. Every target cell changes only from absent to one validated value; no ancestor or passive subtree is rebuilt or replaced.

For an object at the root of an object `resolve` application, Lemmas 1 and 2 establish every activated field-resolver obligation, while the induction hypotheses establish closure below every cell. There is no object-type resolver activation or implicit root bridge obligation in the canonical model. A fixture-generated node loader is simply the `T$Bridge.$node` field resolver whose input fragment selects passive sibling `$id`; its loaded object is completed through the same retained-target induction as every other object value.
